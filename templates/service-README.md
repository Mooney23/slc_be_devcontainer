...
This directory contains the dev container configuration for this service. It inherits from the shared [devcontainer-base](https://github.com/Mooney23/slc_be_devcontainer) image, which provides Python, Neovim, Claude Code, an egress firewall, and common dev tools.

For full documentation on the security model, firewall behavior, and architecture, see the [devcontainer-base README](https://github.com/Mooney23/slc_be_devcontainer?tab=readme-ov-file#security-model).

## Prerequisites

Before your first use, complete the one-time host setup described in the [devcontainer-base setup guide](https://github.com/Mooney23/slc_be_devcontainer?tab=readme-ov-file#2-set-up-the-host-environment-one-time-per-developer). In summary, you need:
...
The Claude Code agent inside this container gets its skills from two sources, merged at startup:

- **Skills baked into the base image** — shipped with the image and updated via `docker pull`. See the [Claude skills section in the base repo](https://github.com/Mooney23/slc_be_devcontainer?tab=readme-ov-file#claude-skills) for the current list of baked skills.
- **Your own host skills** — your `~/.claude/skills/` on the host, flattened and mounted read-only.

Both are merged into the directory Claude actually reads (`~/.claude/skills`, a tmpfs). On a name collision **host-wins** — your personal skill overrides a baked one of the same name. Your global `~/.claude/CLAUDE.md` is resolved and mounted the same way, so the agent picks up your user-scope instructions.

The plumbing lives in **this service's `devcontainer.json`** (not the base image):
...
- mounts: `…/.claude/skills-host` (read-only staging), `…/.claude/skills` (a writable tmpfs the merge writes into), and a `…/.claude/CLAUDE.md` overlay.

> ⚠ **Keep these mounts in sync with the base image.** Unlike startup logic (which arrives via `docker pull`), these mounts are part of *this* `devcontainer.json`. If the base image's skills-merge changes, update the mounts here too — e.g. an older single read-only mount directly on `~/.claude/skills` makes the merge fail with `Permission denied` and your skills won't load. When in doubt, diff your `devcontainer.json` against the base repo's `templates/devcontainer.json`.
...
To handle this, the baked startup script (`dev-post-start.sh`) installs a cron job that re-resolves your `firewall-extras.sh` domains every 5 minutes and adds any new IPs to the firewall whitelist (add-only). This is automatic — no action needed beyond listing the domain in `firewall-extras.sh`. Domains with stable IPs (e.g. `bitbucket.org`) are simply re-resolved to the same addresses, so there's no harm in listing them. See [Phase 6 in the base repo](https://github.com/Mooney23/slc_be_devcontainer?tab=readme-ov-file#phase-6-periodic-dns-refresh-cdn-ip-rotation) for the rationale.

### Fully replacing the firewall script

If you need to change the firewall logic itself (not just add domains), you can place a full copy of `init-firewall.sh` in this directory. See the [base repo documentation](https://github.com/Mooney23/slc_be_devcontainer?tab=readme-ov-file#fully-replacing-the-firewall-script) for details.
...
>
> **One exception — the Claude mounts.** The skills / `CLAUDE.md` mounts live in *this* `devcontainer.json`, not the image, so they do **not** update on `docker pull`. If the base image's skills-merge changes, sync these mounts too (see [Claude skills & global CLAUDE.md](#claude-skills--global-claudemd)).
