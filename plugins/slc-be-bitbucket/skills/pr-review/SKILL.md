---
name: pr-review
description: Review Bitbucket pull requests via the REST API. Use this skill whenever the user wants to review a PR, view a PR, list PRs, check PR comments, get a diff, or post a review comment on a Bitbucket pull request. Triggers on phrases like "review the PR", "view PR", "show me the PR", "list PRs", "PR comments", "check the pull request", "comment on the PR", or any Bitbucket PR URL. Do NOT use this skill to approve or merge pull requests.
---

Review Bitbucket Cloud pull requests on behalf of the user using the Bitbucket REST API v2.0 — read PRs and their diffs and comments, and post review comments. Approving and merging are out of scope.
