...
lives here: space-separated terms = AND, `?"term"` = OR, `-"term"` = exclude. For
structured (JSON) logs, `--raw-pattern` sends a verbatim pattern — e.g. a JSON metric
filter `{ $.level = "DEBUG" }` for an exact **field** match instead of a substring.
...
- **paginates a `search` to exhaustion** — no missed matches from partial results
  (the bounded `sample`/`profile`/probe modes deliberately stop early instead).
...
Only ever calls `describe_log_groups`, `filter_log_events`, `get_log_events`. Four
subcommands:
...
cwlogs.py search   --group <G> --start S --end E (--term T | --raw-pattern P) [operators] [--probe P]
cwlogs.py profile  --group <G> --start S --end E      # what's filling the group (count/byte share)
```
`search` flags:
| Flag | Meaning |
|---|---|
| `--term` | the string you're looking for **and classifying** (REAL/NOISE) — *or* use `--raw-pattern` |
| `--raw-pattern` | a CloudWatch filterPattern sent **verbatim** — for **JSON field matches** like `{ $.level = "DEBUG" }` or other structured syntax. Mutually exclusive with `--term`; owns the whole pattern (no `--context/--all/--any/--not`); all hits report REAL (server matched precisely) |
...
### `profile` — what's *filling* a log group (cost/volume)
Answers "why is this group so big/expensive?" — groups every line by source and tallies
share, so the real volume drivers stand out.
```bash
cwlogs.py profile --group <G> --start S --end E [--max-events N] [--top N] [--sort bytes|count]
```
| Flag | Meaning |
|---|---|
| `--max-events` | cap events sampled (default 60000) — keeps it free/bounded; warns if the cap truncated coverage |
| `--top` | how many line-types to print (default 20) |
| `--sort` | rank by `bytes` (default, = the bill) or `count` (frequency) |

It groups lines by `filename:lineno` (JSON) or a normalized message prefix (plain text)
and reports **count-share, byte-share, and avg bytes/line**. **Byte-share is what
CloudWatch bills**, so a line can be a tiny % of lines yet a large % of bytes (a full
SQL dump) — count alone hides it. It also **auto-flags and redacts secrets** (DB-URI
passwords, `AWSAccessKeyId`, `Signature=`, `x-amz-security-token`) so the profile never
prints a live credential while still showing which lines leak.
...
- **SANITY** (on zero REAL) — the probe is a fast existence check (`PRESENT`/`ABSENT`,
  stops at the first match): `TRUE NEGATIVE` (probe PRESENT, term genuinely absent) vs.
  `INCONCLUSIVE` (probe ABSENT → wrong group/retention/level; not proof).
...
- **⚠ Secrets in logs.** Applications sometimes log sensitive values (DB passwords,
  ...
  — redact before sharing, and raise the logging itself with the owning team. The
  `profile` subcommand auto-detects and **redacts** these patterns and lists which
  lines leak them.
...
- A convenience server-side OR for *plain* terms (raw JSON patterns already support
  `||`; this would bring it to unstructured terms without a required anchor).
