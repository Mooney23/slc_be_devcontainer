...
description: >-
  ...
  the logs", "is there anything in the logs about...", "why is this log group so big", "what's filling
  the logs / driving the CloudWatch bill", "profile the log volume". ESPECIALLY use it for searches over a wide time
...
| **Query** | the assembled match — term + context/all/any/not (or a verbatim `--raw-pattern`), exactly what will match |
...
- `--raw-pattern '<pattern>'` is an **alternative to `--term`**: a CloudWatch filterPattern sent **verbatim** (no quote-wrapping). Reach for it when these logs are **JSON** and you want an exact **field** match — e.g. `--raw-pattern '{ $.level = "DEBUG" }'` matches only the `level` field, not the substring "DEBUG" anywhere in the line. It owns the whole pattern, so it can't combine with `--term/--context/--all/--any/--not` (`--probe` still works), and its hits are all reported REAL (the server matched precisely — there's no client-side noise to classify).
...
## Profiling a log group — what's *filling* it (cost & volume investigations)

When the question is "why is this log group so big / expensive?" rather than "is X in the logs?", use `profile`. Resolve + confirm the group first (Step 0), then sample the window (bounded, free) and tally every line by source so you see what actually dominates:
```bash
python "$SKILL/scripts/cwlogs.py" profile --group <G> \
  --start 2026-03-09T12:00:00 --end 2026-03-09T12:30:00 --max-events 60000
```
- Groups lines by `filename:lineno` (JSON logs) or a normalized message prefix (plain text), and reports **count-share, byte-share, and avg bytes/line** for each line-type.
- **Byte-share is the metric that maps to the bill** — CloudWatch charges per byte *ingested*, so it's the default sort (`--sort count` to rank by frequency instead). A line can be a tiny fraction of *lines* but a large fraction of *bytes* (e.g. a full SQL-query dump); count alone would hide it.
- **Bounded by `--max-events`** (default 60000) so it stays free and predictable on a multi-TB group — it profiles a representative slice rather than paging the whole window, and **says so when the cap is hit** (coverage may be partial → narrow the window). Use `--top N` to control how many line-types print.
- **Auto-flags and redacts secrets** — lines containing DB-URI passwords, `AWSAccessKeyId`, `Signature=`, or `x-amz-security-token` are marked `⚠ SECRET` with their values masked, and listed in a dedicated section (so the profile itself never prints a live credential).

Pick a *representative* window — e.g. the active burst right after a cron fires. The relative breakdown is stable even though absolute volume varies by time of day.
...
The `--probe` distinguishes them: it checks whether events of the relevant *type* **exist** in the same group+window — a fast existence check that stops at the first match, so it stays cheap even on a high-volume probe like `INFO`. If the probe is **PRESENT** and your term is absent → **true negative**. If the probe is **ABSENT** → **inconclusive**, and you must widen/redirect before concluding. Always state which one you're claiming.
...
The `profile` subcommand assists here: it **auto-detects and redacts** these patterns (DB-URI passwords, `AWSAccessKeyId`, `Signature=`, `x-amz-security-token`) and lists exactly which log lines leak them — a fast way to surface credential-logging bugs without exposing the values themselves.
