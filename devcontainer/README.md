...
```
<repo root>/
├── README.md                        # north-star overview (start here)
├── docs/
│   └── direction.md                 # design rationale / north star
├── .claude-plugin/
│   └── marketplace.json             # declares the `slc-plugins` plugin marketplace
├── plugins/                         # Claude skills, shipped as marketplace plugins
│   ├── slc-be-aws-ops/
│   ├── slc-be-knowledge/
│   ├── slc-be-dev/
│   ├── slc-be-bitbucket/
│   └── notes-workflow/
└── devcontainer/                    # the dev-container infra — this guide
    ├── README.md                    # this file
    ├── base-image/
    │   ├── Dockerfile
    │   ├── init-firewall.sh
...
# Optional — only needed for Bitbucket access (mainly the PR-read skill).
# Not required to use the devcontainer; leave unset if you don't use it.
export BITBUCKET_EMAIL="you@yourcompany.com"
export BITBUCKET_TOKEN="your-atlassian-api-token"
```
...
cp /path/to/claude_utils/scripts/post-start.sh .devcontainer/post-start.sh
...
Skills reach the container two ways:

1. **Plugin marketplace (primary).** This repo *is* the `slc-plugins` Claude Code plugin marketplace. A developer installs it **on their host**, and the container inherits the plugins for free — `~/.claude` is bind-mounted in, and that includes `~/.claude/plugins/`. Nothing is baked into the image and nothing is copied per-service: update a skill, push to this repo, and developers pick it up with a normal plugin update. → **Install commands, the capability catalog, and the `--sparse` details live in [`../plugins/README.md`](../plugins/README.md).**
2. **Host user-scope loose skills.** A developer's own ad-hoc skills under `~/.claude/skills/` (not packaged as a plugin) still reach the container. `initializeCommand` flattens them on the host into `~/.cache/.claude-skills-resolved` and mounts that read-only at `~/.claude/skills-host`; `dev-post-start.sh` then copies each skill dir into a `tmpfs` at `~/.claude/skills` — the dir Claude Code actually reads, since `CLAUDE_CONFIG_DIR=/home/dev/.claude`.

### How skills are mounted into the container
...
```
                                       (host ~/.claude is bind-mounted in)
~/.claude/plugins/      (marketplace plugins, installed on host)   ----->  read directly
~/.claude/skills-host/  (host loose skills, RO staging)            --copy-->  ~/.claude/skills  (tmpfs)
```

Only the loose host skills are copied; marketplace plugins are simply visible through the bind-mounted `~/.claude`. The copy is whole-skill-dir replacement (`rm -rf` the target dir, then `cp`) into the tmpfs, so a re-run never leaves a half-merged hybrid. Nothing is written back to the host's `~/.claude/skills`, and nothing lands in `/workspace`.

Docker mounts a `tmpfs` **root-owned**, but the copy runs as the non-root `dev` user, so `dev-post-start.sh` `chown`s `~/.claude/skills` to the current user first — otherwise it fails with `Permission denied`. (Equivalent alternative if you'd rather keep it declarative: add `tmpfs-mode=1777` to the tmpfs mount in `devcontainer.json`.)

**Per-service requirement:** the copy logic is in the base image (propagates via `docker pull`), but the two mounts it depends on — the read-only `skills-host` staging mount and the `tmpfs` over `~/.claude/skills` — live in each service's `devcontainer.json` (see the template). The tmpfs matters because `~/.claude` itself is bind-mounted from the host: without it, the copy would write loose skills back into the developer's real `~/.claude/skills`. Marketplace plugins need neither mount — they ride in through the `~/.claude/plugins/` bind mount automatically.
